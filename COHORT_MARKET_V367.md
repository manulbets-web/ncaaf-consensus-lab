# NCAAF Consensus Lab v3.6.7 — Manual Cohort + Market Shelf

## v3.6.7 Patrick preferred strategy

The one-click Patrick recipe now emphasizes volume, chronological validation, and redundancy control rather than a very large exact search. It first considers up to 43 currently-posting models, ranked by discovery ATS with at least 60 discovery bets per model. Using discovery data only, it greedily keeps up to 15 models whose model-minus-market edges are not near-duplicates at an absolute correlation ceiling of 0.90. The latest 15 usable completed weeks are held out. Exact 3–6 model combinations are then searched inside that diverse core, each combination must have at least 250 discovery bets, combinations are ranked by discovery ATS, and the top 60 are frozen for the existing finalist/META analysis. The 0.75-SD search anchor remains unchanged.
...
v3.6.7 reframes the production workflow around a fixed, interpretable cohort rather than exhaustive combination search.
...
## v3.6.7 exact historical cohort bet audit
