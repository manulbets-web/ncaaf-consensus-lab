    ...
    *,
    ...
    for gi, game_key in enumerate(pred.index):
        ...
        rows.append({
            ...
            "active_combo_ids": " | ".join(f"C{int(x[1])}" for x in active),
            "active_community_ids": " | ".join(str(int(x)) for x in sorted({z[0] for z in active})),
...
def historical_meta_bets(
    data: pd.DataFrame,
    analysis: dict,
    *,
    method: str = "Diversified META",
    standard_price: int = -110,
    scores: pd.DataFrame | None = None,
) -> pd.DataFrame:
    """Return the exact historical bets generated by a frozen META setup.

    The supplied ``analysis`` must be the output of :func:`analyze_finalist_portfolio`.
    Its discovery-selected META threshold is applied unchanged to both the discovery
    and holdout game frames.  This function is therefore an audit/export helper;
    it never re-selects combinations, communities, or k from game outcomes.
    """
    if not isinstance(analysis, dict) or not analysis:
        return pd.DataFrame()
    frames = analysis.get("meta_frames", {})
    selected = analysis.get("meta_selected", pd.DataFrame())
    if not isinstance(frames, dict) or not isinstance(selected, pd.DataFrame):
        return pd.DataFrame()
    qk = selected[selected.get("method", pd.Series(dtype=str)).astype(str).eq(str(method))] if len(selected) else pd.DataFrame()
    k = pd.to_numeric(pd.Series([qk.iloc[0].get("selected_k")]), errors="coerce").iloc[0] if len(qk) else np.nan
    if not np.isfinite(k):
        return pd.DataFrame()
    min_units = int(analysis.get("min_meta_communities", 2)) if str(method) == "Diversified META" else 2

    meta_cols = [c for c in [
        "game_key", "road", "home", "market_snapshot_label", "market_reference_source"
    ] if c in data.columns]
    game_meta = data[meta_cols].drop_duplicates("game_key") if "game_key" in meta_cols else pd.DataFrame()
    win_return = 100.0 / abs(float(standard_price)) if standard_price < 0 else float(standard_price) / 100.0
    rows = []
    for period_key, period_label in (("discovery", "Discovery"), ("holdout", "Holdout")):
        frame = frames.get((str(method), period_key), pd.DataFrame())
        if not isinstance(frame, pd.DataFrame) or frame.empty:
            continue
        d = frame.copy()
        for c in ["meta_edge", "meta_signal", "cover", "market_margin", "actual_margin", "meta_mean", "meta_sd", "active_units", "active_combos"]:
            if c in d.columns:
                d[c] = pd.to_numeric(d[c], errors="coerce")
        gate = d["active_units"].fillna(0).ge(min_units)
        qualifies = gate & (d["meta_signal"].notna() | np.isinf(d["meta_signal"])) & d["meta_signal"].ge(float(k)) & d["meta_edge"].notna() & d["cover"].notna()
        d = d.loc[qualifies].copy()
        if d.empty:
            continue
        if not game_meta.empty:
            d = d.merge(game_meta, on="game_key", how="left")
        for r in d.itertuples(index=False):
            edge = float(getattr(r, "meta_edge"))
            market = float(getattr(r, "market_margin"))
            actual = float(getattr(r, "actual_margin"))
            meta_mean = float(getattr(r, "meta_mean"))
            road = str(getattr(r, "road", "") or "")
            home = str(getattr(r, "home", "") or "")
            bet_home = edge > 0
            bet_team = home if bet_home else road
            bet_line = -market if bet_home else market
            fair_line = -meta_mean if bet_home else meta_mean
            cover_home = actual - market
            bet_cover = cover_home if bet_home else -cover_home
            result = "W" if bet_cover > 1e-12 else "L" if bet_cover < -1e-12 else "P"
            ...
            if abs(bet_line) < 0.05:
                ...
                side_type = "Favorite" if bet_line < 0 else "Underdog"
            rows.append({
                "period": period_label,
                "season": int(getattr(r, "season")),
                "week": int(getattr(r, "week")),
                "game_key": str(getattr(r, "game_key")),
                "road": road,
                "home": home,
                "game": f"{road} @ {home}" if road or home else str(getattr(r, "game_key")),
                "bet": bet,
                "bet_team": bet_team,
                "bet_type": side_type,
                "bet_line": float(bet_line),
                "market_home_margin": market,
                "meta_home_margin": meta_mean,
                "meta_fair_line_on_bet_side": float(fair_line),
                "edge_points": abs(edge),
                "meta_sd": float(getattr(r, "meta_sd")),
                "edge_over_sd": float(getattr(r, "meta_signal")),
                "meta_k": float(k),
                "active_communities": int(getattr(r, "active_units")),
                "active_combos": int(getattr(r, "active_combos")),
                "community_ids": str(getattr(r, "active_community_ids", "") or ""),
                "combo_ids": str(getattr(r, "active_combo_ids", "") or ""),
                "actual_home_margin": actual,
                "bet_cover_margin": float(bet_cover),
                "result": result,
                "unit_result": float(units),
                "line_source": str(getattr(r, "market_snapshot_label", getattr(r, "market_reference_source", "")) or ""),
            })
    ...
    if scores is not None and len(scores):
        ...
        if needed.issubset(sc.columns):
            for c in ["season", "week", "vscore", "hscore"]:
                sc[c] = pd.to_numeric(sc[c], errors="coerce")
            def slug(x):
                z = str(x or "").lower().replace("&", " and ")
                z = re.sub(r"[^a-z0-9]+", " ", z)
                return re.sub(r"\s+", " ", z).strip()
            sc["away_slug"] = sc["road"].map(slug); sc["home_slug"] = sc["home"].map(slug)
            sc = sc.drop_duplicates(["season", "week", "away_slug", "home_slug"])
            out["away_slug"] = out["road"].map(slug); out["home_slug"] = out["home"].map(slug)
            out = out.merge(
                sc[["season", "week", "away_slug", "home_slug", "vscore", "hscore"]],
                on=["season", "week", "away_slug", "home_slug"], how="left"
            )
            def score_label(r):
            ...
            out["final_score"] = out.apply(score_label, axis=1)
            out = out.drop(columns=["away_slug", "home_slug"], errors="ignore")
    if "final_score" not in out.columns:
        out["final_score"] = ""

    order = pd.Categorical(out["period"], categories=["Discovery", "Holdout"], ordered=True)
    out = out.assign(_period_order=order).sort_values(
        ["season", "week", "_period_order", "game"], ascending=[False, False, True, True]
