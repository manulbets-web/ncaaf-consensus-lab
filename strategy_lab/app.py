    ...
    ui.navset_card_tab(
        ...
        ui.nav_panel(
            ...
            ui.card(
                ...
                ui.layout_columns(
                    ui.input_select(
                        "meta_hist_period_filter", "Period shown",
                        choices={"All":"All historical bets", "Discovery":"Discovery only", "Holdout":"Holdout only"},
                        selected="All",
                    ),
                    ui.input_select(
                        "meta_hist_result_filter", "Rows shown",
                        choices={"All":"All results", "W":"Wins only", "L":"Losses only", "P":"Pushes only"},
                        selected="All",
                    ),
                    ui.input_select(
                        "meta_hist_side_filter", "Bet type",
                ...
                ui.layout_columns(
                    ui.value_box("Bets", ui.output_text("meta_hist_bets")),
                    ui.value_box("W-L-P", ui.output_text("meta_hist_record")),
                    ui.value_box("ATS", ui.output_text("meta_hist_ats")),
                    ui.value_box("Units", ui.output_text("meta_hist_units")),
                    ui.value_box("ROI", ui.output_text("meta_hist_roi")),
                    col_widths=(2,3,2,2,3),
                ),
                ui.p(ui.strong("Overview by period"), class_="mb-1"),
...
def server(input, output, session):
    ...
    def _meta_hist_filtered(*, apply_result_filter: bool = True, apply_side_filter: bool = True) -> pd.DataFrame:
        d = _meta_hist_data().copy()
        if d.empty:
            return d
        period = str(input.meta_hist_period_filter() or "All")
        if period in {"Discovery", "Holdout"}:
            d = d[d["period"].astype(str).eq(period)].copy()
        if apply_result_filter:
            result = str(input.meta_hist_result_filter() or "All")
            if result in {"W", "L", "P"}:
                d = d[d["result"].astype(str).eq(result)].copy()
        if apply_side_filter:
    ...
    def _meta_hist_summary(d: pd.DataFrame | None = None) -> dict:
        x = _meta_hist_filtered() if d is None else d.copy()
        if x is None or x.empty:
            return {"bets":0, "wins":0, "losses":0, "pushes":0, "ats":np.nan, "units":0.0, "roi":np.nan}
        wins = int((x["result"] == "W").sum())
        losses = int((x["result"] == "L").sum())
        pushes = int((x["result"] == "P").sum())
        bets = wins + losses
        units = float(pd.to_numeric(x["unit_result"], errors="coerce").fillna(0).sum())
        return {
            "bets":bets, "wins":wins, "losses":losses, "pushes":pushes,
            "ats": wins / bets if bets else np.nan,
    ...
    @render.text
    def meta_hist_status():
        a = _active_committee_analysis()
        if not a:
            return "No active META setup. Run Patrick's recommended strategy or apply a selected custom finalist portfolio first."
        d = _meta_hist_data()
        combos = len(strategy.get().get("combinations", []) or [])
        models = len(strategy.get().get("model_ids", []) or [])
        communities = int(a.get("overlap_summary", {}).get("communities", 0) or 0)
        k = _selected_meta_k("Diversified META")
        label = str(strategy.get().get("label", "Active META portfolio"))
        return (
            f"{label} | {combos} finalist combinations | {models} unique models | "
            f"{communities} independent communities | frozen META k = {k:.2f} SD | "
            f"{len(d):,} exact qualifying historical rows before display filters. "
            "Discovery contributed to setup selection; Holdout remained frozen."
        )

    @render.text
    def meta_hist_bets():
        return f"{_meta_hist_summary()['bets']:,}"

    @render.text
    def meta_hist_record():
        z = _meta_hist_summary()
        return f"{z['wins']}-{z['losses']}-{z['pushes']}"

    @render.text
    def meta_hist_ats():
        v = _meta_hist_summary()["ats"]
        return "—" if not np.isfinite(v) else f"{100*v:.1f}%"

    @render.text
    def meta_hist_units():
        return f"{_meta_hist_summary()['units']:+.2f}u"

    @render.text
    def meta_hist_roi():
        v = _meta_hist_summary()["roi"]
        return "—" if not np.isfinite(v) else f"{100*v:+.1f}%"

    @render.data_frame
    def meta_hist_summary_table():
        d = _meta_hist_data().copy()
        if d.empty:
            return render.DataGrid(pd.DataFrame())
        rows = []
        for label, g in [("Discovery", d[d["period"].eq("Discovery")]), ("Holdout", d[d["period"].eq("Holdout")]), ("Combined · descriptive", d)]:
            z = _meta_hist_summary(g)
            rows.append({
                "Period":label, "Bets":z["bets"], "Wins":z["wins"], "Losses":z["losses"], "Pushes":z["pushes"],
                "ATS %":100*z["ats"] if np.isfinite(z["ats"]) else np.nan,
                "Units":z["units"], "ROI %":100*z["roi"] if np.isfinite(z["roi"]) else np.nan,
            })
        out = pd.DataFrame(rows)
        for c in ["ATS %", "Units", "ROI %"]:
            out[c] = pd.to_numeric(out[c], errors="coerce").round(2)
        return render.DataGrid(out, filters=False, height="190px")

    def _meta_hist_display_frame(*, apply_filters: bool = True) -> pd.DataFrame:
        d = (_meta_hist_filtered() if apply_filters else _meta_hist_data().copy())
        if d.empty:
            return d
        ...
        d["Units"] = pd.to_numeric(d["unit_result"], errors="coerce").round(3)
        d = d.rename(columns={
            "period":"Period", "season":"Season", "week":"Week", "game":"Game", "bet":"Bet",
            "bet_type":"Side type", "active_communities":"Communities", "active_combos":"Active combos",
            "community_ids":"Community IDs", "combo_ids":"Active combo IDs", "final_score":"Final score",
    ...
    @render.data_frame
    def meta_hist_bets_table():
        return render.DataGrid(_meta_hist_display_frame(apply_filters=True), filters=True, height="700px")

    @render.download(filename="active_meta_historical_bets.csv")
    def download_meta_hist():
        yield _meta_hist_display_frame(apply_filters=False).to_csv(index=False)
    ...
    def committee_current_spread_context_table():
        ...
        for x in cur.itertuples(index=False):
            ...
            meta_sd = pd.to_numeric(pd.Series([getattr(x, "meta_total_sd", np.nan)]), errors="coerce").iloc[0]
            meta_signal = pd.to_numeric(pd.Series([getattr(x, "meta_signal_sd", np.nan)]), errors="coerce").iloc[0]
            meta_k = pd.to_numeric(pd.Series([getattr(x, "meta_k", np.nan)]), errors="coerce").iloc[0]
            rows.append({
                "Game": f"{away} @ {home}",
                ...
                "Standardized edge (×SD)": round(float(meta_signal), 2) if np.isfinite(meta_signal) else np.nan,
                "k": round(float(meta_k), 2) if np.isfinite(meta_k) else np.nan,
    ...
    @render.data_frame
    def committee_meta_k_profile_table():
        a = _active_committee_analysis()
        d = a.get("meta_discovery_grid", pd.DataFrame()).copy() if a else pd.DataFrame()
        if not isinstance(d, pd.DataFrame) or d.empty:
            return render.DataGrid(pd.DataFrame())
        d = d[d["method"].astype(str).eq("Diversified META")].copy()
        if d.empty:
            return render.DataGrid(d)
        chosen = _selected_meta_k("Diversified META")
        d["Selected"] = np.isclose(pd.to_numeric(d["k"], errors="coerce"), float(chosen))
        d = _pct_frame(d, ["ats_pct", "roi", "wilson_low"])
        keep = [c for c in ["Selected", "k", "scorable_games", "bets", "wins", "losses", "ats_pct", "roi", "wilson_low"] if c in d.columns]
        d = d[keep].rename(columns={
            "k": "k (SD)", "scorable_games": "Scorable games", "bets": "Bets",
            "wins": "Wins", "losses": "Losses", "ats_pct": "ATS %",
            "roi": "ROI %", "wilson_low": "Wilson LB %",
        })
        return render.DataGrid(d, filters=False, height="330px")
    ...
    def strategy_combo_summary_table():
        ...
        if "meta_mean_home_margin" in d.columns:
            ...
            d["META raw edge (pts)"] = pd.to_numeric(d.get("meta_edge_home"), errors="coerce").abs().round(2)
            d["META consensus SD (pts)"] = pd.to_numeric(d.get("meta_total_sd"), errors="coerce").round(2)
            d["Standardized edge (×SD)"] = pd.to_numeric(d.get("meta_signal_sd"), errors="coerce").round(2)
        else:
            d["Diversified META ± SD"] = "—"
            d["META decision"] = "—"
            d["META raw edge (pts)"] = np.nan
            d["META consensus SD (pts)"] = np.nan
            d["Standardized edge (×SD)"] = np.nan
        keep = [
            "Game", "PT market", "Line used", "Portfolio mean ± SD", "Diversified META ± SD",
            "META raw edge (pts)", "META consensus SD (pts)", "Standardized edge (×SD)",
            "independent_communities", "meta_k", "META decision", "portfolio_combos_used",
        ...
        return render.DataGrid(d[keep].rename(columns={
            ...
            "independent_communities": "Independent communities", "meta_k": "META k",
    ...
    def strategy_combo_detail_table():
        ...
        out = d[keep].rename(columns={
            ...
            "combo_models": "Models", "signal_sd": "Standardized edge (×SD)",
