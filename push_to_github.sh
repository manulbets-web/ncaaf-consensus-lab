...
# v3.5.23: release rebuilds must never delete the verified PredictionTracker
...
echo "Preflight: verifying v3.5.23 Patrick preset..."
python - <<'PYVERIFY'
from pathlib import Path
...
if refresh_missing:
    raise SystemExit("REFUSING TO PUSH: stale v3.5.23 refresh code detected:\n  " + "\n  ".join(refresh_missing))
...
line_missing = [x for x in ["fixed_bet_repricing", "bet_set_overlap", "signal_migration_detail", "opening_line_qc", "run_line_specific_pipelines"] if x not in line_text]
if line_missing:
    raise SystemExit("REFUSING TO PUSH: stale v3.5.23 line-movement module detected:\n  " + "\n  ".join(line_missing))
print("Verified: Top 35 | Wilson | min 25 | sizes 3-6 | ATS rank | 50 finalists | Jaccard 0.50 | exposure audit | fixed-bet repricing | signal migration/CLV | independent line pipelines | strict PT mirror + snapshots")
PYVERIFY

git add .
if ! git diff --cached --quiet; then
  git commit -m "Deploy NCAAF Consensus Lab v3.5.23"
else
