...
# v3.5.24: release rebuilds must never delete the verified PredictionTracker
...
echo "Preflight: verifying v3.5.24 Patrick preset..."
python - <<'PYVERIFY'
from pathlib import Path
...
if refresh_missing:
    raise SystemExit("REFUSING TO PUSH: stale v3.5.24 refresh code detected:\n  " + "\n  ".join(refresh_missing))
...
if line_missing:
    raise SystemExit("REFUSING TO PUSH: stale v3.5.24 line-movement module detected:\n  " + "\n  ".join(line_missing))
print("Verified: Top 35 | Wilson | min 25 | sizes 3-6 | ATS rank | 50 finalists | Jaccard 0.50 | exposure audit | Open-line anomaly QC | frozen usable-week split | fixed-bet repricing | signal migration/CLV | independent line pipelines | strict PT mirror + snapshots")
PYVERIFY

git add .
if ! git diff --cached --quiet; then
  git commit -m "Deploy NCAAF Consensus Lab v3.5.24"
else
