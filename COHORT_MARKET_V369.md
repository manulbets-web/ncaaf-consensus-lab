...
## v3.6.9 Patrick preferred strategy

## Corrected two-stage combination search

v3.6.9 keeps the corrected v3.6.8 two-stage ordering: the full ATS/high-volume candidate pool enters Stage 1, every 3–6 model subset is searched on discovery data, and only then is a ~15-model compatibility core learned from repeated participation in strong discovery combinations plus discovery-only edge-correlation redundancy. A second exact 3–6 search confirms combinations inside that core. The core selection remains completely discovery-only.
...
The v3.6.9 change is the final ranking step. After the discovery-only core is frozen, every discovery-eligible confirmation combination is evaluated on the latest 15 usable completed weeks when the core search contains at most 25,000 candidates. For the preferred 15-model core, this covers all 9,828 possible 3–6 model subsets. Larger custom cores use the top 25,000 discovery leaders for bounded validation ranking. Patrick's preferred final score is 50% discovery ATS + 50% holdout ATS, with at least 25 holdout bets, and the top 60 are retained. Discovery-only ranking remains available as a custom option, as does a 50/50 Wilson-lower-bound rank. Once holdout performance participates in the final rank, that period is validation/tuning data rather than an untouched final test; repeated chronological validation and future prospective weeks remain the clean OOS checks.
...
v3.6.9 reframes the production workflow around a fixed, interpretable cohort rather than exhaustive combination search.
...
## v3.6.9 exact historical cohort bet audit
